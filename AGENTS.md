...
4. **Documentation & Hygiene Principles:**
   - **Accuracy:** every claim a document makes about the system must be accurate — and complete within its scope: a claim that omits part of the truth misleads, and counts as an inaccuracy. The truth is the intended (target) implementation, not necessarily the current source. Reviews verify claim by claim against the source, not by keyword sweep, hunting inaccuracies. A doc↔code mismatch is a finding to surface, never to auto-resolve: the document may be stale, the code may have deviated from intent, or the document may legitimately describe a target not yet built. Only the user rules on which.
   ...
   - **Terminology — agent vs client:** "AI agent" (or "the agent") is the intelligence that acts — calls tools, reads output, recovers from failures. "MCP client" is the software that connects — transport, configuration, the approval guardrail. Do not write "AI client" or "AI tools": each names neither precisely.
   - **Trim test for details:** Evaluate every detail, category, or distinction against the design statements. If it has no bearing on the design (test: if it changed or vanished, would any design statement change?), propose it to the user as a candidate for deletion. If it is only indirectly relevant (an altitude issue), propose stating the underlying general constraint directly rather than enumerating specific classes or variations.
